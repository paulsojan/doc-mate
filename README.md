...
## Replace DocMate with your project name

Let's say that the project name is `Pump`. Execute the command below to replace
all occurrences of `DocMate` with `Pump`.

```bash
perl -pi -w -e 's/DocMate/Pump/g;' $(git ls-files)
```
...
app and once the PR is merged to main, it will be deployed to production instance. The neetoDeploy instance for DocMate
